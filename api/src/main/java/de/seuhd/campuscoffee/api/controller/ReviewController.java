...
public class ReviewController extends CrudController<Review, ReviewDto, Long> {
    ...
    @GetMapping("/filter")
    public @NonNull ResponseEntity<List<ReviewDto>> filter(@RequestParam("pos_id") Long posId,
            @RequestParam("approved") Boolean approved) {
        List<Review> filteredReviews = reviewService.filter(posId, approved);
        List<ReviewDto> filteredReviewDtos = new ArrayList<>();

        for (Review review : filteredReviews) {
            filteredReviewDtos.add(reviewDtoMapper.fromDomain(review));
        }

        return ResponseEntity.ok(filteredReviewDtos);
    }

    public @NonNull ResponseEntity<ReviewDto> approve(@PathVariable Long id, @RequestParam("user_id") Long userId) {
        Review targetReview = reviewService.getById(id);
        targetReview = reviewService.approve(targetReview, userId);

        return ResponseEntity.ok(reviewDtoMapper.fromDomain(targetReview));
    }
}
